...
//		gs.add(goalState1); 
		...
		Heuristic heuristicFunction = new EmptyHeuristic(gs);//new EmptyHeuristic();//new MultiAgentHeuristic(mapmg,singleAgentSolutions);
...
//		tieBreakingOrder.add(Objectives.Probability); // really just here so I can get this too
		...
		ActionSelector actionSelection = new ActionSelectorGreedyLowerBound(tieBreakingOrder,true);//new ActionSelectorGreedyBoundsDiff(tieBreakingOrder);
...
//
		...
		thts.runThrough(new ActionSelectorGreedyLowerBound(tieBreakingOrder,true),resultsLocation);
