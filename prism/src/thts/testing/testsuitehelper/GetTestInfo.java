...
public class GetTestInfo {
    ...
    public HashMap<String, TestSuite> readTestsAndWriteToCSV() {
        ...
        try {
            PrintWriter writer = new PrintWriter(testinfoloc + "allTests/allTests_new.csv");
    ...
    public HashMap<String, TestSuite> filterTestSuitesFSP90R8G8(String tid, HashMap<String, TestSuite> testSuites) {
        HashMap<String, TestSuite> filteredTestSuites = new HashMap<String, TestSuite>();
        for (String testSuiteID : testSuites.keySet()) {
            TestSuite testSuite = testSuites.get(testSuiteID);
            for (String testSetID : testSuite.testSets.keySet()) {
                if (testSetID.contentEquals(tid)) {
                    ...
                    if (!filteredTestSuite.hasTestSet(tid)) {
                        ...
                        for (TestSuiteReadWrite test : ts.tests) {
                            if ((test.numRobots == 8 || test.numGoals == 8) && test.fsp == 90)
                                fts.addTest(test);
                        }
                        filteredTestSuite.addTestSet(tid, fts);
                    }
    ...
    public HashMap<String, TestSuite> filterTestSuitesFSP90ExceptFailstates(String tnid, String doorstnid,HashMap<String, TestSuite> testSuites) {
        String tid;
        HashMap<String, TestSuite> filteredTestSuites = new HashMap<String, TestSuite>();
        for (String testSuiteID : testSuites.keySet()) {
            if(testSuiteID.contentEquals("Doors"))
                tid = doorstnid;
            else
                tid = tnid;
            TestSuite testSuite = testSuites.get(testSuiteID);
