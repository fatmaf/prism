...
public class RunAllConfigsOnSmallExamplePaper {
    ...
    public static void runExample(boolean explore) {
        ...
        int maxruns = 100;
        String resSuffix = "small_example_runs_"+(explore?"explored":"unexplored") + maxruns;
                ...
                try {
                    ...
                    if (!dryrun)
                        runconfig.run(resFolderExt, config,
                                2, 3, filename, false, resSuffix, "_mult", maxruns, 0, 1);
                    testsSofar += maxruns;
    ...
    public static void runSmallExampleSelectedConfig() {
        ...
        for (int i = 0; i < configs.size(); i++) {
            ...
            try {
                System.out.println("\n\nRunning configuration " + config.getConfigname() + " - " + i + "/" + configs.size() + "\n");
                runconfig.run(resFolderExt, config,
                        2, 4, filename, false, resSuffix, "_mult", maxruns, 0, 1);
                testsSofar += maxruns;
