...
public class InvestigatingTieBreaksEtc {
    ...
    public static ArrayList<Configuration> getSelectedConfigsLUGreedy_ASBU(boolean timeBound, boolean dointervalvi, long timeLimit) {
    ...
    public static ArrayList<Configuration> getSelectedConfigseLUGreedy_ASBU(boolean timeBound, boolean dointervalvi, long timeLimit) {
        ArrayList<Configuration> configs = new ArrayList<>();
    ...
    public static ArrayList<Configuration> getSelectedConfigsLUGreedy(boolean timeBound, boolean dointervalvi, long timeLimit) {
        ...
        Configuration greedyLU_L = new ConfigLluGreedy(false, false, false, false);
        configs.add(greedyLU_L);

        Configuration greedyLURandom_L = new ConfigLluGreedyRandom(false, false, false, false);
        configs.add(greedyLURandom_L);

        Configuration greedyLU_P = new ConfigLUGreedy(false, false, false, false);
        configs.add(greedyLU_P);

        Configuration greedyLURandom_P = new ConfigLUGreedyRandom(false, false, false, false);
        configs.add(greedyLURandom_P);

        Configuration greedyLU_Lx = new ConfigLluGreedy(false, false, false, false);
        ((ConfigLluGreedy) greedyLU_Lx).doGreedyPolActSel();
        configs.add(greedyLU_Lx);

        Configuration greedyLURandom_Lx = new ConfigLluGreedyRandom(false, false, false, false);
        ((ConfigLluGreedyRandom) greedyLURandom_Lx).doGreedyPolActSel();
        configs.add(greedyLURandom_Lx);

        Configuration greedyLU_Px = new ConfigLUGreedy(false, false, false, false);
        ((ConfigLUGreedy) greedyLU_Px).doGreedyPolActSel();
        configs.add(greedyLU_Px);

        Configuration greedyLURandom_Px = new ConfigLUGreedyRandom(false, false, false, false);
        ((ConfigLUGreedyRandom) greedyLURandom_Px).doGreedyPolActSel();
        configs.add(greedyLURandom_Px);


        Configuration greedyLU_Lxy = new ConfigLluGreedy(false, false, false, false, false, false);
        ((ConfigLluGreedy) greedyLU_Lxy).doGreedyPolActSel();
        configs.add(greedyLU_Lxy);

        Configuration greedyLURandom_Lxy = new ConfigLluGreedyRandom(false, false, false, false, true, false);
        ((ConfigLluGreedyRandom) greedyLURandom_Lxy).doGreedyPolActSel();
        configs.add(greedyLURandom_Lxy);

        Configuration greedyLU_Pxy = new ConfigLUGreedy(false, false, false, false, true, false);
        ((ConfigLUGreedy) greedyLU_Pxy).doGreedyPolActSel();
        configs.add(greedyLU_Pxy);

        Configuration greedyLURandom_Pxy = new ConfigLUGreedyRandom(false, false, false, false, true, false);
        ((ConfigLUGreedyRandom) greedyLURandom_Pxy).doGreedyPolActSel();
        configs.add(greedyLURandom_Pxy);


        Configuration greedyLU_Lxyz = new ConfigLluGreedy(false, false, false, false, true, true);
        ((ConfigLluGreedy) greedyLU_Lxyz).doGreedyPolActSel();
        configs.add(greedyLU_Lxyz);

        Configuration greedyLURandom_Lxyz = new ConfigLluGreedyRandom(false, false, false, false, true,
                true);
        ((ConfigLluGreedyRandom) greedyLURandom_Lxyz).doGreedyPolActSel();
        configs.add(greedyLURandom_Lxyz);

        Configuration greedyLU_Pxyz = new ConfigLUGreedy(false, false, false, false, true,
                true);
        ((ConfigLUGreedy) greedyLU_Pxyz).doGreedyPolActSel();
        configs.add(greedyLU_Pxyz);

        Configuration greedyLURandom_Pxyz = new ConfigLUGreedyRandom(false, false, false, false, true,
                true);
        ((ConfigLUGreedyRandom) greedyLURandom_Pxyz).doGreedyPolActSel();
        configs.add(greedyLURandom_Pxyz);
    ...
    public static ArrayList<Configuration> getSelectedConfigseLUGreedy(boolean timeBound, boolean dointervalvi, long timeLimit) {
        ...
        Configuration greedyeLU_L = new ConfigLeLUGreedy(false, false, false, false);
        configs.add(greedyeLU_L);

        Configuration greedyeLURandom_L = new ConfigLeLUGreedyRandom(false, false, false, false);
        configs.add(greedyeLURandom_L);

        Configuration greedyeLU_P = new ConfigeLUGreedy(false, false, false, false);
        configs.add(greedyeLU_P);

        Configuration greedyeLURandom_P = new ConfigeLUGreedyRandom(false, false, false, false);
        configs.add(greedyeLURandom_P);

        Configuration greedyeLU_Lx = new ConfigLeLUGreedy(false, false, false, false);
        ((ConfigLeLUGreedy) greedyeLU_Lx).doGreedyPolActSel();
        configs.add(greedyeLU_Lx);

        Configuration greedyeLURandom_Lx = new ConfigLeLUGreedyRandom(false, false, false, false);
        ((ConfigLeLUGreedyRandom) greedyeLURandom_Lx).doGreedyPolActSel();
        configs.add(greedyeLURandom_Lx);

        Configuration greedyeLU_Px = new ConfigeLUGreedy(false, false, false, false);
        ((ConfigeLUGreedy) greedyeLU_Px).doGreedyPolActSel();
        configs.add(greedyeLU_Px);

        Configuration greedyeLURandom_Px = new ConfigeLUGreedyRandom(false, false, false, false);
        ((ConfigeLUGreedyRandom) greedyeLURandom_Px).doGreedyPolActSel();
        configs.add(greedyeLURandom_Px);


        Configuration greedyeLU_Lxy = new ConfigLeLUGreedy(false, false, false, false, true, false);
        ((ConfigLeLUGreedy) greedyeLU_Lxy).doGreedyPolActSel();
        configs.add(greedyeLU_Lxy);

        Configuration greedyeLURandom_Lxy = new ConfigLeLUGreedyRandom(false, false, false, false, true, false);
        ((ConfigLeLUGreedyRandom) greedyeLURandom_Lxy).doGreedyPolActSel();
        configs.add(greedyeLURandom_Lxy);

        Configuration greedyeLU_Pxy = new ConfigeLUGreedy(false, false, false, false, true, false);
        ((ConfigeLUGreedy) greedyeLU_Pxy).doGreedyPolActSel();
        configs.add(greedyeLU_Pxy);

        Configuration greedyeLURandom_Pxy = new ConfigeLUGreedyRandom(false, false, false, false, true, false);
        ((ConfigeLUGreedyRandom) greedyeLURandom_Pxy).doGreedyPolActSel();
        configs.add(greedyeLURandom_Pxy);

        Configuration greedyeLU_Lxyz = new ConfigLeLUGreedy(false, false, false, false, true, true);
        ((ConfigLeLUGreedy) greedyeLU_Lxyz).doGreedyPolActSel();
        configs.add(greedyeLU_Lxyz);

        Configuration greedyeLURandom_Lxyz = new ConfigLeLUGreedyRandom(false, false, false, false, true, true);
        ((ConfigLeLUGreedyRandom) greedyeLURandom_Lxyz).doGreedyPolActSel();
        configs.add(greedyeLURandom_Lxyz);

        Configuration greedyeLU_Pxyz = new ConfigeLUGreedy(false, false, false, false, true, true);
        ((ConfigeLUGreedy) greedyeLU_Pxyz).doGreedyPolActSel();
        configs.add(greedyeLU_Pxyz);

        Configuration greedyeLURandom_Pxyz = new ConfigeLUGreedyRandom(false, false, false, false, true, true);
        ((ConfigeLUGreedyRandom) greedyeLURandom_Pxyz).doGreedyPolActSel();
        configs.add(greedyeLURandom_Pxyz);
    ...
    public static void runSmallExampleSelConfigs() {
        ...
        ArrayList<Configuration> configs = getSelectedConfigsLUGreedy_ASBU(timeBound, dointervalvi, 0);
        configs.addAll(getSelectedConfigseLUGreedy_ASBU(timeBound, dointervalvi, 0));
        configs.addAll(getSelectedConfigsLUGreedy(timeBound, dointervalvi, 0));
        configs.addAll(getSelectedConfigsLUGreedy_ASBU(timeBound, dointervalvi, 0));
