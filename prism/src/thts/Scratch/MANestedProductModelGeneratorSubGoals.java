...
import thts.HelperClass;
import thts.RewardHelper.RewardCalculation;

//a multiagent nested product model generator with subgoals 
//where subgoals are goals where 1,2,3,... tasks have been generated
public class MANestedProductModelGeneratorSubGoals extends DefaultModelGenerator {// ModelGenerator, RewardGenerator {
	...
	public ArrayList<State> initialStateList = null; 
	
	public MANestedProductModelGeneratorSubGoals(ArrayList<ModulesFileModelGenerator> modelGens,
...
//			combinedRobotState.setValue(r, robotStates.get(r));
	...
	public boolean isSubGoalState(State state,int numTasks)
	{
		boolean acc=true; 
		BitSet daAccs = getDAAccsForState(state); 
		if(safetyDAIndex != -1) {
