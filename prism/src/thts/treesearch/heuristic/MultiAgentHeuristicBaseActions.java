...
//TODO: implement as in the factored mdp paper ? but for now meh 
public class MultiAgentHeuristicBaseActions implements Heuristic {
	...
	public MultiAgentHeuristicBaseActions(MultiAgentNestedProductModelGenerator mapmg,
...
//		if (n.baseActionsForRobot == null)
//			baseActionsForRobot = new ArrayList<>();
		State s = n.getState();
		...
		toret = getStateBoundsAndBaseActions(objs, s, baseActionsForRobot);
//		n.baseActionsForRobot = baseActionsForRobot;
		return toret;
	}
...
//			}
	...
	public HashMap<Objectives, Bounds> getStateBoundsAndBaseActions(ArrayList<Objectives> objs, State s,
			ArrayList<HashMap<Object,BaseActionInfo>> baseActionVals) throws PrismException {
		...
		boolean isAvoid = mapmg.isAvoidState(s);
		boolean isDeadend = isDeadend(s);
		if (baseActionVals != null) {
			ArrayList<ArrayList<Object>> actions = mapmg.getAvailableRobotsActionsInState(s);
			for(int r = 0; r<actions.size(); r++)
			{
				
				HashMap<Object,BaseActionInfo> meh = new HashMap<>(); 
				ArrayList<Object> robotActions = actions.get(r); 
				for(int an = 0 ; an<robotActions.size(); an++)
				{
					meh.put(robotActions.get(an), null);
				}
				baseActionVals.add(meh);
			}
		}
	...
	public HashMap<Objectives, Bounds> getStateBounds(ArrayList<Objectives> objs, State s) throws PrismException {
		return this.getStateBoundsAndBaseActions(objs, s, null);
	}
