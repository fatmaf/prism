...
public class ProductModelGenerator2 implements ModelGenerator,RewardGenerator
{
	...
	public ProductModelGenerator2(ModelGenerator modelGen,RewardGenerator rewardGen, DA<BitSet, ? extends AcceptanceOmega> da, List<Expression> labelExprs, StateVector accProbs, NondetModel prodModel)
	{
	...
	{
		return rewardGen.getNumRewardStructs();
	}

	@Override
	public List<String> getRewardStructNames()
	{
		return rewardGen.getRewardStructNames();
	}

	@Override
	public int getRewardStructIndex(String name)
	{
		return rewardGen.getRewardStructIndex(name);
	}

	@Override
	public RewardStruct getRewardStruct(int i) throws PrismException
	{
		return rewardGen.getRewardStruct(i);
	}

	@Override
	public boolean rewardStructHasTransitionRewards(int i)
	{
		return rewardGen.rewardStructHasTransitionRewards(i);
	}
...
//	}
	...
	public double getStateReward(int r, State state) throws PrismException
	{
		if (isReachAcceptanceGoalState(state) || !Double.isFinite((Double)accProbs.getValue(prodModel.getReachableStates().getIndexOfState(state)))) {
			...
			return rewardGen.getStateReward(r, state.substate(0, numVars - 1));
		}
	}
...
/*		System.out.println("AHAH");
		...
		if (isReachAcceptanceGoalState(state) || !Double.isFinite((Double)accProbs.getValue(prodModel.getReachableStates().getIndexOfState(state)))) {
			...
			return rewardGen.getStateActionReward(r, state.substate(0, numVars - 1), action);
		}
	}
